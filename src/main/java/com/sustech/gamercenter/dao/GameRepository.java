...
//    List<Game> findByTag(String tag);
    ...
    @Query(value = "select * from game g where g.release_date < :today and g.tag=:tag and g.name like CONCAT('%',:name,'%')", nativeQuery = true)
    Page<Game> findByTagOrNameLike(@Param("tag") String tag, @Param("name") String name,
                                   @Param("today") String today, Pageable pageable);

    @Query(value = "select * from game g where g.release_date < :today and and g.tag=:tag", nativeQuery = true)
    Page<Game> findAllByTag(String tag, @Param("today") String today, Pageable pageable);

    @Query(value = "select * from game g where g.release_date < :today and g.name like CONCAT('%',:name,'%')", nativeQuery = true)
    Page<Game> findAllByNameLike(String name, @Param("today") String today, Pageable pageable);

    @Query(value = "select * from game g where g.release_date < :today", nativeQuery = true)
